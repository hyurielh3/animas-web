...
export function ScheduleTab({ animes, schedule, reload, notify }: Props) {
  ...
  return <><div className="page-heading"><p>GUÍA DEL CANAL</p><h1>Programación</h1></div><form className="schedule-form" onSubmit={save}><label>Anime *<select value={animeId} onChange={e => setAnimeId(e.target.value)} required><option value="">Selecciona un anime</option>{animes.map(anime => <option key={anime.id} value={anime.id}>{anime.title}</option>)}</select></label><label>Programación<select value={mode} onChange={e => setMode(e.target.value as 'weekly' | 'special')}><option value="weekly">Recurrente</option><option value="special">Día especial</option></select></label>{mode === 'weekly' ? <fieldset className="weekday-picker"><legend>Días de emisión</legend><div className="weekday-options">{days.map((label, value) => <button type="button" className={selectedDays.includes(value) ? 'selected' : ''} key={label} aria-label={label} aria-pressed={selectedDays.includes(value)} onClick={() => toggleDay(value)}>{label.slice(0, 3)}</button>)}</div><div className="weekday-shortcuts" role="group" aria-label="Selección rápida de días"><span className="weekday-shortcuts-label">Selección rápida</span><button type="button" onClick={selectWeekdays}>Lunes a viernes</button><button type="button" onClick={selectAllDays}>Toda la semana</button><button type="button" className="weekday-clear" onClick={() => setSelectedDays([])}>Quitar selección</button></div></fieldset> : <label>Fecha especial *<input type="date" value={date} onChange={e => setDate(e.target.value)} required /></label>}<label>Inicio *<input type="time" value={start} onChange={e => setStart(e.target.value)} required /></label><label>Final *<input type="time" value={end} onChange={e => setEnd(e.target.value)} required /></label><label>Tipo<select value={type} onChange={e => setType(e.target.value as ContentType)}>{types.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><button>Guardar bloque</button></form><div className="admin-list">{schedule.map(entry => <article key={entry.id}><span><b>{animes.find(anime => anime.id === entry.anime_id)?.title ?? 'Anime eliminado'}</b><small>{entry.schedule_date ? `Especial · ${entry.schedule_date}` : `Recurrente · ${days[entry.day_of_week ?? 0]}`} · {entry.starts_at.slice(0, 5)}—{entry.ends_at.slice(0, 5)} · {types.find(([value]) => value === entry.content_type)?.[1] ?? 'Anime'}</small></span><button className="danger" onClick={() => void remove(entry.id)}>Eliminar</button></article>)}</div></>
}
