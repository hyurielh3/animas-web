...
export default function Admin() {
  ...
  const checkRole = async () => { if (!supabase) { notify('Falta configurar Supabase.'); setChecking(false); return }; const { data: { user } } = await supabase.auth.getUser(); if (!user) { setChecking(false); return }; const { data } = await supabase.from('admin_users').select('user_id').eq('user_id', user.id).maybeSingle(); setAuthorized(Boolean(data)); setChecking(false); if (data) void load(); else notify('Esta cuenta no está autorizada para administrar el canal.') }
  ...
  const login = async (event: FormEvent) => { event.preventDefault(); if (!supabase) return; const { error } = await supabase.auth.signInWithPassword({ email, password }); if (error) notify('No se pudo iniciar sesión.') }
  ...
  return <main className="admin-shell"><header className="admin-top"><a href="/" className="admin-brand">ANIMAS <span>ADMIN</span></a><button className="signout" onClick={() => void supabase?.auth.signOut()}>Cerrar sesión</button></header><div className="admin-layout"><aside aria-label="Secciones del panel">{tabs.map(([id, label]) => <button key={id} className={tab === id ? 'selected' : ''} onClick={() => setTab(id)}>{label}</button>)}</aside><section className="admin-content">{message && <p className="notice" role="status">{message}</p>}{tab === 'animes' && <AnimesTab animes={animes} reload={query => void load(query)} notify={notify} />}{tab === 'schedule' && <ScheduleTab animes={animes} schedule={schedule} reload={() => void load()} notify={notify} />}{tab === 'settings' && <SettingsTab settings={settings} onChanged={() => void load()} notify={notify} />}</section></div></main>
}
